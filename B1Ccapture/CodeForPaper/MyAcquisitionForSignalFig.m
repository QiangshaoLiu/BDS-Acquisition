%Author:LSQ
%Date:2019/4
%Description: 利用学位论文设计的B1C信号捕获算法对采集到的卫星信号数据进行处理，
%             得到收星情况的三维图和二维图.
%Parameters: 
...
[data_num data_order data_AD_in] = textread('201904031124.txt','%d%d%d');

len = length(data_AD_in);
freq = (1:len)./len * 12.4;
...
fft_data_squ = abs(fft(data_AD_in)).^2;
...
%hold on;
%plot3(code_phase,doppler,value1,'k.','markersize',20);     %小黑点标记
%text(code_phase,doppler,value1,['X:',num2str(code_phase,4),char(10),'Y:',...
%    num2str(doppler,4),char(10),'Z:',num2str(value1),char(10)]);  %标值
view(0,0);                        %二维视角
xlabel('码相位延时(码片)');ylabel('多普勒频移(Hz)');zlabel('相关值');
%axis([0 10230 -5000 5000 0 value1+1e4]);
axis([5360 5376 -5000 5000 0 value1+1e4]);
