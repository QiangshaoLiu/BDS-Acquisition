%此程序用于验证ASPeCT捕获算法，对不同的影响因子下的BOC(1,1)和BOC(6,1)进行了分析
...
title('ASPeCT捕获算法重构的相关函数');

figure(2)
plot(index, Aspect, 'r', index, Aspect_1, 'g', index, Aspect_2, 'b');
hold on
plot(index, Aspect_3, 'k', index, Aspect_4, 'y', index, Aspect_5, 'm');
%grid on;
xlabel('码片');
ylabel('归一化相关函数');
axis([-2.5 2.5 -0.6 1.4]);
legend('beta=0.8','beta=0.9','beta=1.0','beta=1.1','beta=1.2','beta=1.3');
title('不同加权系数下的重构相关函数');
